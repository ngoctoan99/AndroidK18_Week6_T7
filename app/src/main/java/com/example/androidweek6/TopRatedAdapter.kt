...
class TopRatedAdapter(private val onClickListener: OnClickListener) : ListAdapter<Movie, TopRatedAdapter.TopRatedVH>(TopRatedDiffUtilCallback()) {
    class TopRatedDiffUtilCallback : DiffUtil.ItemCallback<Movie>() {
    ...
    class TopRatedVH private constructor(var binding: ItemFilmBinding) :
        RecyclerView.ViewHolder(binding.root) {
        companion object {
            fun from(parent: ViewGroup): TopRatedVH {
                val layoutInflater = LayoutInflater.from(parent.context)
                val binding = ItemFilmBinding.inflate(layoutInflater, parent, false)
                return TopRatedVH(binding)
            }
        }
